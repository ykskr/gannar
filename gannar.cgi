...
@items=(
	{'name','強化槍','order',1,'gettype',0,'max',9,'val',1,'fid',1,'time',3600,'ename','攻撃力Up',},
	{'name','将軍旗','order',2,'gettype',0,'max',9,'val',0.3,'fid',2,'time',3600,'ename','影響力Up',},
	{'name','栄養剤','order',4,'gettype',0,'max',9,'val',10,'fid',3,'time',3600,'ename','栄養剤NG',},
	{'name','鋼鉄柵','order',7,'gettype',0,'max',9,'need',2,},
	{'name','掘削機','order',8,'gettype',0,'max',9,'need',4,},
	{'name','土砂袋','order',9,'gettype',0,'max',9,'need',7,},
	{'name','芭蕉扇','order',6,'gettype',0,'max',9,},
	{'name','双眼鏡','order',10,'gettype',0,'max',9,'val',1,'val2',3,},
	{'name','帰還書','order',5,'gettype',0,'max',9,},
	{'name','地雷罠','order',0,'gettype',0,'max',9,'short','爾','val',5,'tid',1,},
	{'name','狩猟弓','order',3,'gettype',0,'max',5,'fid',4,'time',1800,'ename','貫通力Up'},
);
...
sub item{
	...
	if($item==0 || $item==1 || $item==10){
		$return.=substr($items[$item]{'ename'},0,-2).'を'.$items[$item]{'time'}.'秒の間増加させます。<br>';
		...
		$txt=&printpt($$map[$posi]{'land'},$posi).'へとワープしました。';
		...
		$$log{'action'}[0]=&printtime(time).' '.&printpl($pl).'が'.$txt;
		if($$map[$$pl{'posi'}]{'belong'}==$$pl{'belong'}){
			$txt=&printcn($$pl{'belong'},'どこか').'に'.$items[$item]{'name'}.'を設置しました。';
			$$map[$$pl{'posi'}]{'trap'}=$items[$item]{'tid'};
		}else{
			$txt=&printcn($$pl{'belong'},'自陣').'以外のため設置に失敗しました。';
		}
		$$log{'action'}[0].=$txt;
		$return.=$txt.'<br>';
	}
...
sub printmap{
	...
	foreach($i=0;$i<$width*$height;$i++){
		...
		$return.=$items[$trapitem[$map[$i]{'trap'}]]{'short'} if $param{'pl'}{'name'} && $map[$i]{'trap'} && $map[$i]{'belong'}==$param{'pl'}{'belong'};
